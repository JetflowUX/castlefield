...
function ServiceCard({ icon, title, description, features, delay }: ServiceCardProps) {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      transition={{ delay }}
      className="group relative rounded-3xl glow-card p-8 cursor-default shadow-sm"
    >
      <div className="w-16 h-16 rounded-2xl bg-background flex items-center justify-center mb-6 border border-border relative z-10">
        {icon}
      </div>
      <h2 className="font-heading text-2xl font-bold text-foreground mb-4 relative z-10">
        {title}
      </h2>
      <p className="text-foreground/60 leading-relaxed mb-6 relative z-10 font-light text-sm md:text-base">
        {description}
      </p>
      <ul className="space-y-3 relative z-10">
        {features.map((feature, fIdx) => (
          <li key={fIdx} className="flex items-center gap-3 text-sm text-foreground/80 font-medium">
      ...
      <div className="container mx-auto">
        ...
        >
          ...
          <p className="text-foreground/60 text-lg font-light">
            Beyond offering premium vehicles, we provide a comprehensive suite of services designed to make your buying and ownership experience exceptional.
          </p>
        </motion.div>
